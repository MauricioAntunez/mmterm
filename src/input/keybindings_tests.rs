...
fn ctrl_w_arrow_left_focuses_left() {
    assert!(matches!(ctrl_w_action(&named(NamedKey::ArrowLeft)), Action::FocusLeft));
}

#[test]
fn ctrl_w_arrow_right_focuses_right() {
    assert!(matches!(ctrl_w_action(&named(NamedKey::ArrowRight)), Action::FocusRight));
}

#[test]
fn ctrl_w_arrow_up_focuses_up() {
    assert!(matches!(ctrl_w_action(&named(NamedKey::ArrowUp)), Action::FocusUp));
}

#[test]
fn ctrl_w_arrow_down_focuses_down() {
    assert!(matches!(ctrl_w_action(&named(NamedKey::ArrowDown)), Action::FocusDown));
}
...
fn ctrl_w_named_escape_returns_none() {
    assert!(matches!(ctrl_w_action(&named(NamedKey::Escape)), Action::None));
}

// ── handle_key_inner — global shortcuts ─────────────────────────────────────

fn insert() -> InputMode { InputMode::Insert }
fn normal() -> InputMode { InputMode::Normal }
fn visual() -> InputMode {
    InputMode::Visual { start_col: 0, start_row: 0, cur_col: 0, cur_row: 0 }
}
...
fn ctrl_dot_from_search_enters_insert() {
    let mode = InputMode::Search { query: String::new() };
...
fn ctrl_shift_arrow_up_scrolls_up_1() {
    let a = handle_key_inner(&named(NamedKey::ArrowUp), true, true, &insert(), 80, 24, false);
    assert!(matches!(a, Action::ScrollUp(1)));
}

#[test]
fn ctrl_shift_arrow_down_scrolls_down_1() {
    let a = handle_key_inner(&named(NamedKey::ArrowDown), true, true, &insert(), 80, 24, false);
    assert!(matches!(a, Action::ScrollDown(1)));
}

#[test]
fn ctrl_shift_page_up_scrolls_half_page() {
    let a = handle_key_inner(&named(NamedKey::PageUp), true, true, &insert(), 80, 24, false);
    assert!(matches!(a, Action::ScrollUp(12)));
}

#[test]
fn ctrl_shift_page_down_scrolls_half_page() {
    let a = handle_key_inner(&named(NamedKey::PageDown), true, true, &insert(), 80, 24, false);
...
fn ctrl_page_up_prev_tab() {
    let a = handle_key_inner(&named(NamedKey::PageUp), true, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::PrevTab));
}

#[test]
fn ctrl_page_down_next_tab() {
    let a = handle_key_inner(&named(NamedKey::PageDown), true, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::NextTab));
}

#[test]
fn shift_page_up_scrolls_full_page() {
    let a = handle_key_inner(&named(NamedKey::PageUp), false, true, &insert(), 80, 24, false);
    assert!(matches!(a, Action::ScrollUp(24)));
}

#[test]
fn shift_page_down_scrolls_full_page() {
    let a = handle_key_inner(&named(NamedKey::PageDown), false, true, &insert(), 80, 24, false);
...
fn insert_escape_sends_esc_byte() {
    let a = handle_key_inner(&named(NamedKey::Escape), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[0x1b]));
}

#[test]
fn insert_enter_sends_cr() {
    let a = handle_key_inner(&named(NamedKey::Enter), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[b'\r']));
}

#[test]
fn insert_backspace_sends_del() {
    let a = handle_key_inner(&named(NamedKey::Backspace), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[0x7f]));
}

#[test]
fn insert_tab_sends_tab_byte() {
    let a = handle_key_inner(&named(NamedKey::Tab), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[b'\t']));
}

#[test]
fn insert_space_sends_space() {
    let a = handle_key_inner(&named(NamedKey::Space), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[b' ']));
}
...
fn insert_ctrl_enter_sends_newline() {
    let a = handle_key_inner(&named(NamedKey::Enter), true, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == &[b'\n']));
}

#[test]
fn insert_arrow_up_normal_cursor() {
    let a = handle_key_inner(&named(NamedKey::ArrowUp), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[A"));
}

#[test]
fn insert_arrow_up_application_cursor() {
    let a = handle_key_inner(&named(NamedKey::ArrowUp), false, false, &insert(), 80, 24, true);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1bOA"));
}

#[test]
fn insert_arrow_down_normal_cursor() {
    let a = handle_key_inner(&named(NamedKey::ArrowDown), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[B"));
}

#[test]
fn insert_arrow_right_normal_cursor() {
    let a = handle_key_inner(&named(NamedKey::ArrowRight), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[C"));
}

#[test]
fn insert_arrow_left_normal_cursor() {
    let a = handle_key_inner(&named(NamedKey::ArrowLeft), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[D"));
}

#[test]
fn insert_home_normal() {
    let a = handle_key_inner(&named(NamedKey::Home), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[1~"));
}

#[test]
fn insert_home_application_cursor() {
    let a = handle_key_inner(&named(NamedKey::Home), false, false, &insert(), 80, 24, true);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1bOH"));
}

#[test]
fn insert_end_normal() {
    let a = handle_key_inner(&named(NamedKey::End), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[4~"));
}

#[test]
fn insert_page_up_sends_csi_5() {
    let a = handle_key_inner(&named(NamedKey::PageUp), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[5~"));
}

#[test]
fn insert_page_down_sends_csi_6() {
    let a = handle_key_inner(&named(NamedKey::PageDown), false, false, &insert(), 80, 24, false);
    assert!(matches!(a, Action::SendToPty(ref v) if v == b"\x1b[6~"));
}

#[test]
fn insert_delete_sends_csi_3() {
    let a = handle_key_inner(&named(NamedKey::Delete), false, false, &insert(), 80, 24, false);
...
fn insert_f1_through_f4() {
    ...
    for (k, expected) in cases {
        ...
        assert!(matches!(a, Action::SendToPty(ref v) if v.as_slice() == *expected), "F key mismatch");
    }
}
...
fn normal_escape_enters_insert() {
    let a = handle_key_inner(&named(NamedKey::Escape), false, false, &normal(), 80, 24, false);
    assert!(matches!(a, Action::SetMode(InputMode::Insert)));
}
...
fn normal_page_up_scrolls_full_grid() {
    let a = handle_key_inner(&named(NamedKey::PageUp), false, false, &normal(), 80, 24, false);
    assert!(matches!(a, Action::ScrollUp(24)));
}

#[test]
fn normal_page_down_scrolls_full_grid() {
    let a = handle_key_inner(&named(NamedKey::PageDown), false, false, &normal(), 80, 24, false);
    assert!(matches!(a, Action::ScrollDown(24)));
}
...
fn visual_at(sc: usize, sr: usize, cc: usize, cr: usize) -> InputMode {
    InputMode::Visual { start_col: sc, start_row: sr, cur_col: cc, cur_row: cr }
}

fn vis_col_row(a: Action) -> (usize, usize) {
    match a {
        Action::SetMode(InputMode::Visual { cur_col, cur_row, .. }) => (cur_col, cur_row),
...
fn visual_h_moves_left() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, row) = vis_col_row(handle_key_inner(&char_key("h"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 4);
    assert_eq!(row, 5);
}
...
fn visual_l_moves_right() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, row) = vis_col_row(handle_key_inner(&char_key("l"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 6);
    assert_eq!(row, 5);
}
...
fn visual_k_moves_up() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, row) = vis_col_row(handle_key_inner(&char_key("k"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 5);
    assert_eq!(row, 4);
}
...
fn visual_j_moves_down() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, row) = vis_col_row(handle_key_inner(&char_key("j"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 5);
    assert_eq!(row, 6);
}
...
fn visual_zero_jumps_to_col_zero() {
    ...
    let (col, _) = vis_col_row(handle_key_inner(&char_key("0"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 0);
}
...
fn visual_dollar_jumps_to_last_col() {
    ...
    let (col, _) = vis_col_row(handle_key_inner(&char_key("$"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 79);
}
...
fn visual_g_jumps_to_row_zero() {
    ...
    let (_, row) = vis_col_row(handle_key_inner(&char_key("g"), false, false, &mode, 80, 24, false));
    assert_eq!(row, 0);
}
...
fn visual_arrow_left_moves_cursor() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, _) = vis_col_row(handle_key_inner(&named(NamedKey::ArrowLeft), false, false, &mode, 80, 24, false));
    assert_eq!(col, 4);
}
...
fn visual_arrow_right_moves_cursor() {
    let mode = visual_at(0, 0, 5, 5);
    let (col, _) = vis_col_row(handle_key_inner(&named(NamedKey::ArrowRight), false, false, &mode, 80, 24, false));
    assert_eq!(col, 6);
}
...
fn visual_home_jumps_to_col_zero() {
    let mode = visual_at(0, 0, 10, 5);
    let (col, _) = vis_col_row(handle_key_inner(&named(NamedKey::Home), false, false, &mode, 80, 24, false));
    assert_eq!(col, 0);
}
...
fn visual_end_jumps_to_last_col() {
    let mode = visual_at(0, 0, 0, 0);
    let (col, _) = vis_col_row(handle_key_inner(&named(NamedKey::End), false, false, &mode, 80, 24, false));
    assert_eq!(col, 79);
}
...
fn visual_h_at_col_zero_clamps() {
    ...
    let (col, _) = vis_col_row(handle_key_inner(&char_key("h"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 0);
}
...
fn visual_k_at_row_zero_clamps() {
    let mode = visual_at(0, 0, 5, 0);
    let (_, row) = vis_col_row(handle_key_inner(&char_key("k"), false, false, &mode, 80, 24, false));
    assert_eq!(row, 0);
}
...
fn visual_l_at_last_col_clamps() {
    ...
    let (col, _) = vis_col_row(handle_key_inner(&char_key("l"), false, false, &mode, 80, 24, false));
    assert_eq!(col, 79);
}
...
fn visual_start_coords_preserved_on_move() {
    ...
    match a {
        Action::SetMode(InputMode::Visual { start_col, start_row, .. }) => {
...
fn search_mode_returns_none() {
    let mode = InputMode::Search { query: "foo".into() };
    let a = handle_key_inner(&char_key("a"), false, false, &mode, 80, 24, false);
    assert!(matches!(a, Action::None));
}
