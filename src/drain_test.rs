...
#[test]
fn discard_signal_clears_pending_output() {
    use std::sync::atomic::Ordering;

    let (entry, tx) = make_pane_entry();
    // Queue many chunks before parser thread can drain them
    for _ in 0..100 {
        tx.send(b"AAAAAAAAA\r\n".to_vec()).unwrap();
    }
    // Set discard signal
    entry.discard_signal.store(true, Ordering::Release);
    // Give parser thread time to see signal and drain
    std::thread::sleep(std::time::Duration::from_millis(100));
    // Signal should be cleared by the parser thread
    assert!(
        !entry.discard_signal.load(Ordering::Acquire),
        "discard_signal should be cleared by parser thread"
    );
}
