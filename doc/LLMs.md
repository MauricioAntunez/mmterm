...
- Modal scopes: `default_keymap()` populates `ModeClass::Normal` rows (`i`→enter_insert_mode, `v`→enter_visual_mode, `q`→close_pane, `/`→search_open, `n`→search_next, `N`→search_prev, `j`→scroll_line_down, `k`→scroll_line_up, PageUp/PageDown→scroll_page_up/down) and `ModeClass::Visual` rows (`w`/`b`/`e`→visual_word_*, `y`→copy, `Y`→visual_yank_line, `o`→visual_swap_anchor, `v`→visual_anchor, `q`→enter_insert_mode). Dispatch: `handle_normal`/`handle_visual` look up `ModeClass::Normal`/`Visual` FIRST with `mods = ∅` and a case-preserved glyph token (`token_from_key(key, false)`), so the lookup is modifier-agnostic and case-sensitive (`N`≠`n`, `Y`≠`y`). A miss keeps the hardcoded grammar: Normal `Escape`→Insert only; Visual `Escape`→Insert plus all cursor movement (`h l k j 0 $ g G`, arrows, Home/End, Page boundary). Parser preserves glyph case for `normal:`/`visual:` scopes (Global stays lowercased). Arbitrary new chord prefixes remain out of scope.
